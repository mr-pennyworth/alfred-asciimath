...
def get_app_version():
  with open('AsciiMath Renderer.xcodeproj/project.pbxproj') as f:
...
def set_app_version(old_v, new_v):
  with open('AsciiMath Renderer.xcodeproj/project.pbxproj', 'r') as f:
    new_content = f.read().replace(
      f'MARKETING_VERSION = {old_v}',
      f'MARKETING_VERSION = {new_v}'
    )
  with open('AsciiMath Renderer.xcodeproj/project.pbxproj', 'w') as f:
    f.write(new_content)
